...
async def initialize_drone(drone: System):
    ...
    await asyncio.sleep(5)  # 等待无人机到达起飞高度

    # 设置初始悬停点，为切换到offboard模式做准备
    print("-- Setting initial setpoint")
    await drone.offboard.set_velocity_ned(VelocityNedYaw(0.0, 0.0, 0.0, 90.0))
...
async def avoid_obstacle_with_velocity_ned_yaw(drone, block_averages):
    if block_averages['inner'][0] > 40:
        ...
        await drone.offboard.set_velocity_ned(VelocityNedYaw(0.0, 1.0, 0.0, 90.0))
        await asyncio.sleep(1)
    else:
        if max(block_averages['middle']) == all_block_averages['middle'][0]:
            # 左上方路徑清晰
            print("Path is clear in the upper left!")
            await drone.offboard.set_velocity_ned(VelocityNedYaw(1.0, 0.0, 0.4, 90.0))
            await asyncio.sleep(1)
        elif max(block_averages['middle']) == all_block_averages['middle'][1]:
            # 正上方路徑清晰
            print("Path is clear above!")
            await drone.offboard.set_velocity_ned(VelocityNedYaw(0.0, 0.0, 0.4, 90.0))
            await asyncio.sleep(1)
