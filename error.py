...
# 新的代碼塊替換原有的模型加載方式
state_dict = torch.load(args.model_dir)  # 加載預訓練模型的狀態字典
new_state_dict = OrderedDict()

for k, v in state_dict.items():
    name = k[7:] if k.startswith('module.') else 'module.'+k  # 適應DataParallel的命名規則
...
total_time = end_time - start_time  # 計算總運行時間
